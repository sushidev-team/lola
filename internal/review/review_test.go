...
func TestClassifyRunErrQuota(t *testing.T) {
	// Quota cue in stderr on a nonzero exit → ErrQuota (not ErrExit), and no
	// stdout/stderr is echoed into the error (actionable class only).
	...
	if e := classifyRunErr(nil, nil, "", "You are out of reviews for this billing period.", time.Second); !errors.Is(e, ErrQuota) {
