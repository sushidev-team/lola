        ...
        <tbody>
          {#each filtered as p (p.number)}
            ...
            <tr class="group cursor-pointer border-t border-edge/30 hover:bg-sel/50">
              <td class="num px-3 py-2 text-right {p.alreadyOpen ? 'text-faint' : ''}" onclick={() => openShell(p)}>#{p.number}</td>
              ...
              <td class="px-3 py-2 text-sm text-faint" onclick={() => openShell(p)}>{p.author || "—"}</td>
              <td class="px-3 py-2 font-mono text-sm text-faint" onclick={() => openShell(p)}>
