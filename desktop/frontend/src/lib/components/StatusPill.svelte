<script lang="ts">
  ...
  let {
    ...
    delivery = "",
    interpreted = "",
  }: { status: string; agentState?: string; delivery?: string; interpreted?: string } = $props();
  ...
  const badge = $derived(
    interpreted ? "" : showAgentBadge(status, agentState, delivery) ? agentBadge(agentState) : "",
  );
</script>
...
<span class="inline-flex items-center whitespace-nowrap">
  ...
  {#if interpreted}
    ...
    <span class="ml-0.5 text-[10px] text-orange">≈{statusLabel(interpreted)}</span>
  {:else if badge}
